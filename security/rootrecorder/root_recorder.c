...
static DEFINE_SPINLOCK(recordlist_lock);
...
static int obstruct_by_memlist(struct record_list_node *rln)
{
	...
	spin_lock_irqsave(&recordlist_lock, flags);
	...
	rlistn = kmalloc(sizeof(*rlistn), GFP_ATOMIC);
	if (!rlistn) {
		ret = -ENOMEM;
		goto unlock;
	}
...
unlock:
	spin_unlock_irqrestore(&recordlist_lock, flags);
	return ret;
}
...
static struct record_list_node *record_dequeue(void)
{
	...
	spin_lock_irqsave(&torecord_lock, flags);
	...
	list_del(&rln->node);
unlock:
	spin_unlock_irqrestore(&torecord_lock, flags);
	return rln;
}
...
void record_illegal_root(const char *behavior, int cap)
{
	...
	rln = kmalloc(sizeof(*rln), GFP_ATOMIC);
	if (!rln)
		return;
