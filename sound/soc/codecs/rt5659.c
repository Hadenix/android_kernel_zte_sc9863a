...
#define pr_fmt(fmt) "[ASoC:RT5659] "fmt
...
#define NEXT_EXT_PARAMS_SET(data) \
	(struct realtek_extend_cell *)((char *)(data) + \
	(data)->header.regs_num * sizeof(struct realtek_extend_reg) + \
	sizeof(struct realtek_extend_header))
...
static struct reg_default init_list[] = {
	...
	/* Headset mic (IN1)
	 * Set BST1 to 36dB, BST2 to 15.75dB.
	 */
	{RT5659_IN1_IN2,		0x4025},
	{RT5659_IN3_IN4,		0x2500}, /* Set BST3 to 15.75dB */
	{RT5659_IL_CMD_3,	0x9ed4},
	{RT5659_BIAS_CUR_CTRL_4,	0xaaa8},
	/* Switch off class D AMP in default. */
	{RT5659_CLASSD_2,	0x1154},
};
#define RT5659_INIT_REG_LEN ARRAY_SIZE(init_list)

static struct reg_default init_list_one_gpio_detect[] = {
	{RT5659_PWR_ANLG_2,			0x0000}, /*JD3 power down */
	{RT5659_IRQ_CTRL_2,			0x0000}, /*close JD3 detection*/
...
static struct reg_default init_list_jd3_and_gpio_detect[] = {
...
int rt5659_headset_detect(struct snd_soc_codec *codec, int jack_insert)
{
	...
	if (jack_insert) {
		/* fix micbias1 power remain */
		snd_soc_update_bits(codec, RT5659_DUMMY_2, 0x10, 0x00);
		...
		/* fix micbias1 power remain */
		snd_soc_update_bits(codec, RT5659_DUMMY_2, 0x10, 0x10);
	}
...
int rt5659_check_jd_status(struct snd_soc_codec *codec)
{
	...
	if (rt5659->pdata.hp_det_use_one_pin) {
		if (rt5659->pdata.gpio_jd1_sel > 0)
			mask = 0x1000;
		if (rt5659->pdata.gpio_jd2_sel > 0)
			mask = 0x0800;
	} else {
		/* todo: add jd1, jd2 and so on cases.
		 * fixed on jd3 now.
		 */
		mask = 0x0080;
	}
	val = snd_soc_read(codec, RT5659_INT_ST_1) & mask;

	if (!val) {  /* Jack insert */
		pr_debug("%s-Jack In(mask: %#x)\n", __func__, mask);
		return 1;
	}
	/* jack out */
	pr_debug("%s-Jack Out(mask: %#x)\n", __func__, mask);
	return 0;
}
...
static const char * const rt5658_fw_stat_sel[] = {
	"Not update", "Updated"
};

static const SOC_ENUM_SINGLE_DECL(rt5658_fw_stat_enum, 0, 0,
				  rt5658_fw_stat_sel);

static void rt5659_extend_params_fw_load(struct snd_soc_codec *codec,
					 const struct firmware *fw)
{
	struct rt5659_priv *rt5659 = snd_soc_codec_get_drvdata(codec);
	int i, j, k;
	struct realtek_extend *fw_params;
	struct realtek_extend_cell *data;
	struct realtek_extend_reg *default_settings;
	struct addr_mask {
		uint16_t addr;
		uint16_t mask;
	} *uniq_regs = NULL;
	size_t data_size = 0;
	uint32_t regs_num = 0;
	uint32_t raw_regs_num = 0;
	unsigned int val;
	static int default_regs_init;

	if (!fw) {
		pr_info("%s fw is NULL\n", __func__);
		return;
	}

	/* TODO: add a magic check. */

	fw_params = (struct realtek_extend *)fw->data;
	pr_info("%s fw->size: %zu, params set num: %u, size: %u\n", __func__,
		fw->size, fw_params->count, fw_params->size);

	rt5659->extend_params_count = fw_params->count;
	data_size = fw_params->size - sizeof(fw_params->size) -
		sizeof(fw_params->count);
	kfree(rt5659->extend_params);
	rt5659->extend_params = kzalloc(data_size, GFP_KERNEL);
	if (!rt5659->extend_params)
		goto free_firmware_uniq_regs;
	memcpy(rt5659->extend_params, fw_params->data, data_size);

	rt5659->extend_params_ready = 0;

	/* Calculate the total number of registers in fw.*/
	data = rt5659->extend_params;
	for (i = 0; i < rt5659->extend_params_count; i++) {
		pr_debug("Set[%d] mode: %02u ~ %02u\n", i,
			 data->header.start_mode, data->header.end_mode);
		for (j = 0; j < data->header.regs_num; j++) {
			pr_debug("       reg%#06x=%#06x, mask: %#06x\n",
				 data->reg[j].addr, data->reg[j].val,
				 data->reg[j].mask);
		}
		raw_regs_num += data->header.regs_num;
		data = NEXT_EXT_PARAMS_SET(data);
	}

	uniq_regs = kcalloc(raw_regs_num, sizeof(struct addr_mask),
			      GFP_KERNEL);
	if (!uniq_regs)
		goto free_extend_params;

	/* Calculate how many unique registers will be operated. */
	data = rt5659->extend_params;

	/* 1. Get the first register set. */
	regs_num = data->header.regs_num;
	for (i = 0; i < regs_num; i++) {
		uniq_regs[i].addr = data->reg[i].addr;
		uniq_regs[i].mask = data->reg[i].mask;
	}
	data = NEXT_EXT_PARAMS_SET(data);

	/* 2. Process the rest of register sets. */
	for (i = 1; i < rt5659->extend_params_count; i++) {
		for (j = 0; j < data->header.regs_num; j++) {
			for (k = 0; k < regs_num; k++) {
				if (uniq_regs[k].addr == data->reg[j].addr) {
					uniq_regs[k].mask |= data->reg[j].mask;
					break;
				}
			}
			if (k == regs_num) {
				uniq_regs[regs_num].addr = data->reg[j].addr;
				uniq_regs[regs_num].mask = data->reg[j].mask;
				regs_num++;
			}
		}
		data = NEXT_EXT_PARAMS_SET(data);
	}
	rt5659->regs_num = regs_num;

	/* Fill the default settings */
	kfree(rt5659->default_settings);
	rt5659->default_settings = kcalloc(regs_num,
					   sizeof(struct realtek_extend_reg),
					   GFP_KERNEL);
	default_settings = rt5659->default_settings;
	if (!default_settings)
		goto free_extend_params;

	/* Get the default values after boot up. */
	if (!default_regs_init) {
		for (i = 0; i < regs_num; i++) {
			val = snd_soc_read(codec, uniq_regs[i].addr);
			if (val < 0) {
				pr_err("%s read reg#%#x failed(%d)!\n",
				       __func__, uniq_regs[i].addr, val);
				default_settings[i].addr = -1;
				continue;
			}
			default_settings[i].addr = uniq_regs[i].addr;
			default_settings[i].val = val;
			default_settings[i].mask = uniq_regs[i].mask;
			pr_debug("default_settngs[%d] reg: %#06x, val: %#06x, mask: %#06x\n",
				 i, default_settings[i].addr,
				 default_settings[i].val,
				 default_settings[i].mask);
		}
		default_regs_init = 1;
	}

	rt5659->extend_params_ready = 1;
	goto free_firmware_uniq_regs;

free_extend_params:
	kfree(rt5659->extend_params);
free_firmware_uniq_regs:
	kfree(uniq_regs);
	release_firmware(fw);
}

static int rt5659_extend_params_put(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_soc_kcontrol_codec(kcontrol);
	int read = ucontrol->value.integer.value[0];
	const struct firmware *fw;
	int ret;

	if (read) {
		ret = request_firmware(&fw, "realtek_codec", codec->dev);
		if (ret != 0) {
			pr_err("ERR: %s Failed to load 'realtek_codec' firmware: %d\n",
			       __func__, ret);
			return ret;
		}
		rt5659_extend_params_fw_load(codec, fw);
	}

	return 0;
}

static int rt5659_extend_params_get(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_soc_kcontrol_codec(kcontrol);
	struct rt5659_priv *rt5659 = snd_soc_codec_get_drvdata(codec);

	if (rt5659->extend_params_ready)
...
static void rt5658_drc_dac_fw_loaded(struct snd_soc_codec *codec,
				     const struct firmware *fw)
{
	struct rt5659_priv *rt5659 = snd_soc_codec_get_drvdata(codec);
	int i;
	struct vbc_fw_header *header;
	struct eq_drc_dat_format *eq_drc;
	char *offset;
	...
	header = (struct vbc_fw_header *)fw->data;
	pr_info("%s fw->size=%zu, num_mode=%d, len of one mode=%d\n", __func__,
		fw->size, header->num_mode, header->len);

	if (header->len != sizeof(struct eq_drc_dat_format)) {
		pr_err("ERR: %s the data is illegal.\n", __func__);
		goto RELEASE_FIRMWARE;
	}

	if (rt5659->eq_drc_num_mode != header->num_mode) {
		if (header->num_mode <= 0) {
			pr_err("ERR: %s num_mode is %d.\n",
			       __func__, header->num_mode);
			goto RELEASE_FIRMWARE;
		}
		kfree(rt5659->eq_drc);
		rt5659->eq_drc = kcalloc(header->num_mode,
			sizeof(struct eq_drc_dat_format),
			GFP_KERNEL);
		if (!rt5659->eq_drc) {
			pr_err("%s kmalloc eq_drc fail\n", __func__);
			goto RELEASE_FIRMWARE;
		}
		rt5659->eq_drc_num_mode = header->num_mode;
	}

	offset = (char *)(fw->data);
	offset += sizeof(struct vbc_fw_header);
	for (i = 0; i < rt5659->eq_drc_num_mode; i++) {
		eq_drc = (struct eq_drc_dat_format *)offset;
		memcpy(rt5659->eq_drc + i, eq_drc,
			sizeof(struct eq_drc_dat_format));
		offset += sizeof(struct eq_drc_dat_format);
	}
...
static int rt5658_eq_drc_memory_put(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_soc_kcontrol_codec(kcontrol);
	int read = ucontrol->value.integer.value[0];
	...
	if (read) {
		ret = request_firmware(&fw, "codec", codec->dev);
		if (ret != 0) {
			pr_err("ERR: %s Failed to load 'codec' firmware: %d\n",
			       __func__, ret);
			return ret;
		}
		rt5658_drc_dac_fw_loaded(codec, fw);
	}

	return 0;
}

static int rt5658_eq_drc_memory_get(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_soc_kcontrol_codec(kcontrol);
	struct rt5659_priv *rt5659 = snd_soc_codec_get_drvdata(codec);
...
static int rt5658_eq_drc_mode_get(struct snd_kcontrol *kcontrol,
				  struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_soc_kcontrol_codec(kcontrol);
	struct rt5659_priv *rt5659 = snd_soc_codec_get_drvdata(codec);

	ucontrol->value.integer.value[0] = rt5659->mode;

	return 0;
}

static void rt5658_eq_dac_update(struct eq_dat_format3 *eq,
				 struct snd_soc_dapm_widget *w)
{
	int ch, i;
	WORD reg_num;
	struct compose_codec_register *eq_ctrl_reg;
	struct snd_soc_codec *codec = snd_soc_dapm_to_codec(w->dapm);

	for (ch = EQ_FROM_DACL_5658; ch <= EQ_FROM_DACR_5658; ch++) {
		reg_num = eq->eq_driver_parameter[ch].eq_ctrl_reg_num;
		eq_ctrl_reg = eq->eq_driver_parameter[ch].eq_ctrl_reg;
		pr_debug("%s eq reg num of channel(%d): %u, w->power: %u\n",
			 __func__, ch, reg_num, w->power);
		for (i = 0; i < reg_num; i++) {
			/* bypass EQ enable register if EQ is disabled */
			if (eq_ctrl_reg[i].index == RT5659_DAC_EQ_CTRL_2 &&
			    w->power == 0)
				continue;
			snd_soc_write(codec, eq_ctrl_reg[i].index,
				      eq_ctrl_reg[i].data);
			pr_debug("%s EQ DAC reg#%#06x = %#06x\n", __func__,
				 eq_ctrl_reg[i].index, eq_ctrl_reg[i].data);
		}
	}
}

static void rt5658_eq_adc_update(struct eq_dat_format3 *eq,
				 struct snd_soc_dapm_widget *w)
{
	int i, ch;
	WORD reg_num;
	...
	for (ch = EQ_FROM_ADCL_5658; ch <= EQ_FROM_ADCR_5658; ch++) {
		reg_num = eq->eq_driver_parameter[ch].eq_ctrl_reg_num;
		eq_ctrl_reg = eq->eq_driver_parameter[ch].eq_ctrl_reg;
		pr_debug("%s eq reg num of channel(%d): %u, w->power: %u\n",
			 __func__, ch, reg_num, w->power);
		for (i = 0; i < reg_num; i++) {
			/* bypass EQ enable register if EQ is disabled  */
			if (eq_ctrl_reg[i].index ==
			    RT5659_ADC_EQ_CTRL_2 && w->power == 0)
				continue;
			snd_soc_write(codec, eq_ctrl_reg[i].index,
				      eq_ctrl_reg[i].data);
			pr_debug("%s EQ ADC reg#%#06x = %#06x\n", __func__,
				 eq_ctrl_reg[i].index, eq_ctrl_reg[i].data);
		}
	}
}

static inline int rt5659_apply_ext_default_settings(struct rt5659_priv *rt5659)
{
	int i;
	struct snd_soc_codec *codec = rt5659->codec;
	struct realtek_extend_reg *settings = rt5659->default_settings;

	for (i = 0; i < rt5659->regs_num; i++) {
		if (settings[i].addr == -1) {
			pr_warn("%s default settings[%d] is invalid!\n",
				__func__, i);
			continue;
		}
		snd_soc_update_bits(codec, settings[i].addr, settings[i].mask,
				    settings[i].val);
		pr_debug("extend params default settings %d: reg: %#06x mask: %#06x, val: %#06x\n",
			 i, settings[i].addr, settings[i].mask,
			 settings[i].val);
	}
...
static inline int rt5659_apply_ext_params(struct rt5659_priv *rt5659,
					  struct realtek_extend_cell *params)
{
	int i;
	struct snd_soc_codec *codec = rt5659->codec;
	struct realtek_extend_reg *reg = params->reg;

	for (i = 0; i < params->header.regs_num; i++) {
		snd_soc_update_bits(codec, reg[i].addr, reg[i].mask,
				    reg[i].val);
		pr_debug("extend params settings %d: reg: %#06x mask: %#06x, val: %#06x\n",
			 i, reg[i].addr, reg[i].mask, reg[i].val);
	}

	return 0;
}

static int rt5659_ext_params_process(struct rt5659_priv *rt5659)
{
	int i;
	struct realtek_extend_cell *params;

	if (!rt5659->extend_params_ready) {
		pr_info("Extend params is not ready yet.\n");
		return -EINVAL;
	}

	rt5659_apply_ext_default_settings(rt5659);

	params = rt5659->extend_params;
	for (i = 0; i < rt5659->extend_params_count; i++) {
		if (rt5659->mode >= params->header.start_mode &&
		    rt5659->mode <= params->header.end_mode) {
			pr_debug("mode: %d, start_mode: %u, end_mode: %u\n",
				 rt5659->mode, params->header.start_mode,
				 params->header.end_mode);
			return rt5659_apply_ext_params(rt5659, params);
		}
		params = NEXT_EXT_PARAMS_SET(params);
	}

	return 0;
}

static int rt5658_eq_drc_mode_put(struct snd_kcontrol *kcontrol,
				  struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_soc_kcontrol_codec(kcontrol);
	struct rt5659_priv *rt5659 = snd_soc_codec_get_drvdata(codec);
	...
	struct eq_drc_dat_format *eq_drc;
	struct eq_dat_format3 *eq;
	struct drc_dat_format3 *drc;
	struct compose_codec_register *drc_ctrl_reg;
	bool is_dac_path = false;
	int i;

	rt5659->mode = ucontrol->value.integer.value[0];
	if (rt5659->mode >= rt5659->eq_drc_num_mode) {
		pr_err("ERR: %s the selected mode(%d) is over total mode number(%d)\n",
		       __func__, rt5659->mode, rt5659->eq_drc_num_mode);
		rt5659->mode = 0;
		return -1;
	}

	rt5659_ext_params_process(rt5659);

	if (!rt5659->eq_drc) {
		pr_err("ERR: %s rt5659->eq_drc is NULL\n", __func__);
		return -1;
	}

	eq_drc = rt5659->eq_drc + rt5659->mode;
	eq = &(eq_drc->eq);
	drc = &(eq_drc->drc);
	pr_info("%s eq_en=%u, drc_en=%u, mode=%d\n", __func__,
		eq_drc->eq_switch, eq_drc->drc_switch, rt5659->mode);

	if (!eq) {
		pr_info("%s eq is NULL\n", __func__);
		goto DRC;
	}
	/* Check if EQ path is ADC or DAC */
	switch (eq->eq_ui_parameter.eq_ui_source) {
	/* DAC path*/
	case EQ_FROM_DACL_5658:
	case EQ_FROM_DACR_5658:
		is_dac_path = true;
		pr_debug("%s EQ DAC path\n", __func__);
		/* get "EQ DAC" widget*/
		list_for_each_entry(w, &card->widgets, list) {
			if (strcmp(w->name, "EQ DAC") == 0)
				break;
		}
		break;
	/* ADC path*/
	case EQ_FROM_ADCL_5658:
	case EQ_FROM_ADCR_5658:
		is_dac_path = false;
		pr_debug("%s EQ ADC path\n", __func__);
		/* get "EQ ADC" widget*/
		list_for_each_entry(w, &card->widgets, list) {
			if (strcmp(w->name, "EQ ADC") == 0)
				break;
		}
		break;
	default:
	...
	if (!w) {
		pr_err("ERR: %s no EQ widget\n", __func__);
		return -1;
	}

	if (eq_drc->eq_switch) {
		if (is_dac_path) {
			rt5659->eq_dac_en = true;
			/* Write EQ parameter to codec */
			rt5658_eq_dac_update(eq, w);
		} else {
			rt5659->eq_adc_en = true;
			/* Write EQ parameter to codec */
			rt5658_eq_adc_update(eq, w);
		}
	} else {
		if (is_dac_path)
			rt5659->eq_dac_en = false;
		else
			rt5659->eq_adc_en = false;
	}

DRC:
	if (!eq_drc->drc_switch)
		return 0;

	if (!drc) {
		pr_info("%s drc is NULL\n", __func__);
		return 0;
	}

	/* get "DRC widget*/
	list_for_each_entry(w, &card->widgets, list) {
		if (strcmp(w->name, "DRC") == 0)
			break;
	}
	if (!w) {
		pr_err("ERR: %s no DRC widget\n", __func__);
		return -1;
	}

	pr_debug("%s drc reg num: %u, w->power: %u\n", __func__,
		 drc->alc_driver_parameter.drc_ctrl_reg_num, w->power);

	/* Write DRC parameter to codec */
	drc_ctrl_reg = drc->alc_driver_parameter.drc_ctrl_reg;
	for (i = 0; i < drc->alc_driver_parameter.drc_ctrl_reg_num; i++) {
		/* bypass DRC enable register */
		if ((drc_ctrl_reg[i].index == RT5659_DRC1_CTRL_2 ||
		     drc_ctrl_reg[i].index == RT5659_CROSS_OVER_1 ||
		     drc_ctrl_reg[i].index == RT5659_MULTI_DRC_CTRL) &&
		    w->power == 0)
			continue;
		snd_soc_write(codec, drc_ctrl_reg[i].index,
			      drc_ctrl_reg[i].data);
		pr_debug("%s DRC reg#%#06x = %#06x\n", __func__,
			 drc_ctrl_reg[i].index, drc_ctrl_reg[i].data);
	}

	return 0;
}
static const char * const rt5659_micbias2_power_mode[] = {
		"Disable", "Enable"
};
...
static const char * const rt5659_iis_width_txt[] = {
	"bit16", "bit24"
};

static const struct soc_enum rt5659_iis_width_enum[RT5659_AIFS] = {
	SOC_ENUM_SINGLE(RT5659_AIF1, 0, 2, rt5659_iis_width_txt),
	SOC_ENUM_SINGLE(RT5659_AIF2, 0, 2, rt5659_iis_width_txt),
	SOC_ENUM_SINGLE(RT5659_AIF3, 0, 2, rt5659_iis_width_txt),
};

static int rt5659_iis_width_get(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_soc_kcontrol_codec(kcontrol);
	struct rt5659_priv *rt5659 = snd_soc_codec_get_drvdata(codec);
	struct soc_enum *e = (struct soc_enum *)kcontrol->private_value;
	unsigned int aif = e->reg;

	ucontrol->value.integer.value[0] = rt5659->iis_width[aif];

	return 0;
}

static int rt5659_iis_width_put(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_soc_kcontrol_codec(kcontrol);
	struct rt5659_priv *rt5659 = snd_soc_codec_get_drvdata(codec);
	...
	rt5659->iis_width[aif] = ucontrol->value.integer.value[0];
...
static const struct snd_kcontrol_new rt5659_snd_controls[] = {
	...
	SOC_ENUM_EXT("CODEC Profile Update", rt5658_fw_stat_enum,
		rt5658_eq_drc_memory_get, rt5658_eq_drc_memory_put),
	SOC_ENUM_EXT("Realtek Extend Codec Param Update", rt5658_fw_stat_enum,
		rt5659_extend_params_get, rt5659_extend_params_put),
	SOC_SINGLE_EXT("CODEC Profile Select", 0, 0, 127, 0,
		rt5658_eq_drc_mode_get, rt5658_eq_drc_mode_put),

	SOC_ENUM_EXT("AIF1 IIS Width Select",
		     rt5659_iis_width_enum[RT5659_AIF1],
		     rt5659_iis_width_get, rt5659_iis_width_put),
	SOC_ENUM_EXT("AIF2 IIS Width Select",
		     rt5659_iis_width_enum[RT5659_AIF2],
		     rt5659_iis_width_get, rt5659_iis_width_put),
	SOC_ENUM_EXT("AIF3 IIS Width Select",
		     rt5659_iis_width_enum[RT5659_AIF3],
		     rt5659_iis_width_get, rt5659_iis_width_put),
};
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		/* to decrease noise flow */
		snd_soc_update_bits(codec, RT5659_SILENCE_CTRL, 0x8000, 0x8000);
		snd_soc_update_bits(codec, RT5659_PSV_CTRL, 0x2000, 0x2000);
		snd_soc_update_bits(codec, RT5659_CLASSD_0, 0x2000, 0x2000);
		snd_soc_update_bits(codec, RT5659_DIG_MISC, 0x0020, 0x0020);
		break;
	...
	case SND_SOC_DAPM_POST_PMD:
		...
		/* to decrease noise flow */
		snd_soc_update_bits(codec, RT5659_SILENCE_CTRL, 0x8000, 0x0000);
		snd_soc_update_bits(codec, RT5659_PSV_CTRL, 0x2000, 0x0000);
		snd_soc_update_bits(codec, RT5659_CLASSD_0, 0x2000, 0x0000);
		snd_soc_update_bits(codec, RT5659_DIG_MISC, 0x0020, 0x0000);
		break;
	case SND_SOC_DAPM_POST_PMU:
		msleep(20);
		break;
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMD:
		snd_soc_write(codec, RT5659_DEPOP_1, 0x0000);
		snd_soc_update_bits(codec, RT5659_DUMMY_2, 0x1000, 0x0000);
		break;
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		snd_soc_update_bits(codec, RT5659_PWR_ANLG_2,
			RT5659_PWR_BST2_P | RT5659_PWR_BST2 | RT5659_PWR_MB3,
			RT5659_PWR_BST2_P | RT5659_PWR_BST2 | RT5659_PWR_MB3);
	break;
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		snd_soc_update_bits(codec, RT5659_PWR_ANLG_2,
			RT5659_PWR_BST3_P | RT5659_PWR_BST3 | RT5659_PWR_MB2,
			RT5659_PWR_BST3_P | RT5659_PWR_BST3 | RT5659_PWR_MB2);
		break;
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		snd_soc_update_bits(codec, RT5659_PWR_ANLG_2,
			RT5659_PWR_BST4_P | RT5659_PWR_BST4 | RT5659_PWR_MB2,
			RT5659_PWR_BST4_P | RT5659_PWR_BST4 | RT5659_PWR_MB2);
		break;
...
static int rt5658_eq_adc_event(struct snd_soc_dapm_widget *w,
			       struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_codec *codec = snd_soc_dapm_to_codec(w->dapm);
	struct rt5659_priv *rt5659 = snd_soc_codec_get_drvdata(codec);
	...
	struct eq_drc_dat_format *eq_drc;
	struct eq_dat_format3 *eq;
	struct driver_eq_parameter *eq_param;

	if (rt5659->eq_drc == NULL)
		return 0;

	eq_drc = rt5659->eq_drc + rt5659->mode;
	eq = &(eq_drc->eq);
	eq_param = &eq->eq_driver_parameter[EQ_FROM_ADCL_5658];

	switch (event) {
	case SND_SOC_DAPM_PRE_PMU:
		for (i = 0; i < eq_param->eq_ctrl_reg_num; i++) {
			if (eq_param->eq_ctrl_reg[i].index ==
			    RT5659_ADC_EQ_CTRL_2) {
				snd_soc_write(codec, RT5659_ADC_EQ_CTRL_2,
					      eq_param->eq_ctrl_reg[i].data);
			}
		}
		break;

	case SND_SOC_DAPM_POST_PMD:
		snd_soc_write(codec, RT5659_ADC_EQ_CTRL_2, 0x0000);
		break;
...
static int rt5658_eq_dac_event(struct snd_soc_dapm_widget *w,
			       struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_codec *codec = snd_soc_dapm_to_codec(w->dapm);
	struct rt5659_priv *rt5659 = snd_soc_codec_get_drvdata(codec);
	...
	eq_drc = rt5659->eq_drc + rt5659->mode;
	eq = &(eq_drc->eq);
	eq_param = &eq->eq_driver_parameter[EQ_FROM_DACL_5658];
	...
	case SND_SOC_DAPM_PRE_PMU:
		for (i = 0; i < eq_param->eq_ctrl_reg_num; i++) {
			if (eq_param->eq_ctrl_reg[i].index ==
				...
				snd_soc_write(codec, RT5659_DAC_EQ_CTRL_2,
					      eq_param->eq_ctrl_reg[i].data);
			}
		}
		break;
	...
	case SND_SOC_DAPM_POST_PMD:
		snd_soc_write(codec, RT5659_DAC_EQ_CTRL_2, 0x0000);
		break;
	...
	pr_debug("%s event: %d, RT5659_DAC_EQ_CTRL_2: %#x\n", __func__,
		 event, snd_soc_read(codec, RT5659_DAC_EQ_CTRL_2));

	return 0;
}

static int rt5658_drc_event(struct snd_soc_dapm_widget *w,
			    struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_codec *codec = snd_soc_dapm_to_codec(w->dapm);
	struct rt5659_priv *rt5659 = snd_soc_codec_get_drvdata(codec);
	...
	struct eq_drc_dat_format *eq_drc;
	struct drc_dat_format3 *drc;
	struct driver_alc_parameter2 *drc_param;
	...
	eq_drc = rt5659->eq_drc + rt5659->mode;
	drc = &(eq_drc->drc);
	drc_param = &drc->alc_driver_parameter;

	switch (event) {
	case SND_SOC_DAPM_PRE_PMU:
		for (i = 0; i < drc_param->drc_ctrl_reg_num; i++) {
			/* Write DRC enable register*/
			switch (drc_param->drc_ctrl_reg[i].index) {
			case RT5659_DRC1_CTRL_2:
			case RT5659_CROSS_OVER_1:
			case RT5659_MULTI_DRC_CTRL:
				snd_soc_write(codec,
					      drc_param->drc_ctrl_reg[i].index,
					      drc_param->drc_ctrl_reg[i].data);
				break;
			default:
				break;
			}
	...
	case SND_SOC_DAPM_POST_PMD:
		/* Write DRC register to default value */
		snd_soc_write(codec, RT5659_DRC1_CTRL_2, 0x032c);
		snd_soc_write(codec, RT5659_CROSS_OVER_1, 0x8700);
		snd_soc_write(codec, RT5659_MULTI_DRC_CTRL, 0x0f20);
		break;
...
static int rt5658_adc_use_eq(struct snd_soc_dapm_widget *source,
			     struct snd_soc_dapm_widget *sink)
{
	struct snd_soc_codec *codec = snd_soc_dapm_to_codec(source->dapm);
	struct rt5659_priv *rt5659 = snd_soc_codec_get_drvdata(codec);

	return rt5659->eq_adc_en ? 1 : 0;
}

static int rt5658_dac_use_eq(struct snd_soc_dapm_widget *source,
			     struct snd_soc_dapm_widget *sink)
{
	struct snd_soc_codec *codec = snd_soc_dapm_to_codec(source->dapm);
	struct rt5659_priv *rt5659 = snd_soc_codec_get_drvdata(codec);

	return rt5659->eq_dac_en ? 1 : 0;
}

static int rt5658_use_drc(struct snd_soc_dapm_widget *source,
			  struct snd_soc_dapm_widget *sink)
{
	struct snd_soc_codec *codec = snd_soc_dapm_to_codec(source->dapm);
	struct rt5659_priv *rt5659 = snd_soc_codec_get_drvdata(codec);

	if (rt5659->eq_drc && rt5659->eq_drc->drc_switch)
		return 1;
...
static const struct snd_soc_dapm_widget rt5659_dapm_widgets[] = {
	...
	/* Added following 4 lines by peng.lee@spreadtrum.com */
	SND_SOC_DAPM_AIF_IN("AIF1RX OFLD",
		"AIF1 Playback ofld", 0, SND_SOC_NOPM, 0, 0),
	SND_SOC_DAPM_AIF_IN("AIF1RX FAST",
		"AIF1 Playback fast", 0, SND_SOC_NOPM, 0, 0),
	SND_SOC_DAPM_AIF_IN("AIF1RX VOICE",
		"AIF1 Playback voice", 0, SND_SOC_NOPM, 0, 0),
	SND_SOC_DAPM_AIF_IN("AIF1RX VOIP",
		"AIF1 Playback voip", 0, SND_SOC_NOPM, 0, 0),
	SND_SOC_DAPM_AIF_IN("AIF1RX FM",
		"AIF1 Playback fm", 0, SND_SOC_NOPM, 0, 0),
	SND_SOC_DAPM_AIF_IN("AIF1RX NORMAL",
		"AIF1 Playback normal", 0, SND_SOC_NOPM, 0, 0),
	SND_SOC_DAPM_AIF_OUT("AIF1TX", "AIF1 Capture", 0, SND_SOC_NOPM, 0, 0),
	/* Added following 1 lines by peng.lee@spreadtrum.com */
	SND_SOC_DAPM_AIF_OUT("AIF1TX VOICE",
		"AIF1 Capture voice", 0, SND_SOC_NOPM, 0, 0),
	SND_SOC_DAPM_AIF_OUT("AIF1TX VOIP",
		"AIF1 Capture voip", 0, SND_SOC_NOPM, 0, 0),
	SND_SOC_DAPM_AIF_OUT("AIF1TX NORMAL",
		"AIF1 Capture normal", 0, SND_SOC_NOPM, 0, 0),
	...
	/* EQ and DRC */
	SND_SOC_DAPM_SUPPLY_S("EQ ADC", 6, SND_SOC_NOPM, 0, 0,
		rt5658_eq_adc_event, SND_SOC_DAPM_PRE_PMU |
		SND_SOC_DAPM_POST_PMD),
	SND_SOC_DAPM_SUPPLY_S("EQ DAC", 6, SND_SOC_NOPM, 0, 0,
		rt5658_eq_dac_event, SND_SOC_DAPM_PRE_PMU |
		SND_SOC_DAPM_POST_PMD),
	SND_SOC_DAPM_SUPPLY_S("DRC", 6, SND_SOC_NOPM, 0, 0,
		rt5658_drc_event, SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD),
...
static const struct snd_soc_dapm_route rt5659_dapm_routes[] = {
	...
	{ "Stereo1 ADC Volume L", NULL, "EQ ADC", rt5658_adc_use_eq },
	{ "Stereo1 ADC Volume L", NULL, "DRC", rt5658_use_drc },
	{ "Stereo1 ADC Volume R", NULL, "Stereo1 ADC MIXR" },
	{ "Stereo1 ADC Volume R", NULL, "EQ ADC", rt5658_adc_use_eq },
	{ "Stereo1 ADC Volume R", NULL, "DRC", rt5658_use_drc },
	...
	{ "AIF1TX", NULL, "IF1 ADC" },
	/* Added following 1 lines by peng.lee@spreadtrum.com */
	{ "AIF1TX VOICE", NULL, "IF1 ADC" },
	{ "AIF1TX VOIP", NULL, "IF1 ADC" },
	{ "AIF1TX NORMAL", NULL, "IF1 ADC" },
	{ "AIF2TX", NULL, "IF2 ADC" },
	{ "AIF3TX", NULL, "IF3 ADC" },

	{ "IF1 DAC1", NULL, "AIF1RX" },
	{ "IF1 DAC2", NULL, "AIF1RX" },
	/* Added following 8 lines by peng.lee@spreadtrum.com */
	{ "IF1 DAC1", NULL, "AIF1RX OFLD" },
	{ "IF1 DAC2", NULL, "AIF1RX OFLD" },
	{ "IF1 DAC1", NULL, "AIF1RX FAST" },
	{ "IF1 DAC2", NULL, "AIF1RX FAST" },
	{ "IF1 DAC1", NULL, "AIF1RX VOICE" },
	{ "IF1 DAC2", NULL, "AIF1RX VOICE" },
	{ "IF1 DAC1", NULL, "AIF1RX VOIP" },
	{ "IF1 DAC2", NULL, "AIF1RX VOIP" },
	{ "IF1 DAC1", NULL, "AIF1RX FM" },
	{ "IF1 DAC2", NULL, "AIF1RX FM" },
	{ "IF1 DAC1", NULL, "AIF1RX NORMAL" },
	{ "IF1 DAC2", NULL, "AIF1RX NORMAL" },
	{ "IF2 DAC", NULL, "AIF2RX" },
	{ "IF3 DAC", NULL, "AIF3RX" },
	...
	{ "DAC1 MIXL", NULL, "EQ DAC", rt5658_dac_use_eq },
	{ "DAC1 MIXL", NULL, "DRC", rt5658_use_drc },
	{ "DAC1 MIXR", "Stereo ADC Switch", "Stereo1 ADC Volume R" },
	{ "DAC1 MIXR", "DAC1 Switch", "DAC R1 Mux" },
	{ "DAC1 MIXR", NULL, "DAC Stereo1 Filter" },
	{ "DAC1 MIXR", NULL, "EQ DAC", rt5658_dac_use_eq },
	{ "DAC1 MIXR", NULL, "DRC", rt5658_use_drc },
	...
	struct snd_pcm_hw_params *params, struct snd_soc_dai *dai)
{
	...
	int index = DAI_ID_TO_INDEX(dai->id);

	pr_debug("%s\n", __func__);

	rt5659->lrck[index] = rt5659->pdata.fixed_rate > 0 ?
		rt5659->pdata.fixed_rate : params_rate(params);
	pre_div = get_clk_info(rt5659->sysclk, rt5659->lrck[index]);
	if (pre_div < 0) {
		dev_err(codec->dev, "Unsupported clock setting %d for DAI %d\n",
			rt5659->lrck[index], dai->id);
		return -EINVAL;
	}
	...
	if (rt5659->iis_width[index] == 1)/* 24bits */
		bclk_ms = 1;

	rt5659->bclk[index] = rt5659->lrck[index] * (32 << bclk_ms);

	dev_dbg(dai->dev, "bclk is %dHz and lrck is %dHz\n",
		rt5659->bclk[index], rt5659->lrck[index]);
	dev_dbg(dai->dev, "bclk_ms is %d and pre_div is %d for iis %d\n",
				bclk_ms, pre_div, index);
	...
	if (rt5659->iis_width[index] == 1)/* 24bits */
		val_len = RT5659_I2S_DL_24;
	pr_debug("%s iis width: %d, bclk_ms: %d, val_len: %d\n",
		__func__, rt5659->iis_width[index], bclk_ms, val_len);
...
static int rt5659_set_dai_fmt(struct snd_soc_dai *dai, unsigned int fmt)
{
	...
	case SND_SOC_DAIFMT_CBM_CFM:
		rt5659->master[index] = 1;
		break;
	case SND_SOC_DAIFMT_CBS_CFS:
		rt5659->master[index] = 0;
		break;
	default:
		dev_err(codec->dev, "Invalid fmt: %#x\n", fmt);
		return -EINVAL;
	}
			...
			enum snd_soc_bias_level level)
{
	...
	case SND_SOC_BIAS_STANDBY:
		...
		if (dapm->bias_level == SND_SOC_BIAS_PREPARE) {
			regmap_update_bits(rt5659->regmap, RT5659_PWR_ANLG_1,
					   RT5659_PWR_MB | RT5659_PWR_VREF2, 0);
		}
		break;
...
static int rt5659_reg_init(struct snd_soc_codec *codec)
{
	...
	if (rt5659->pdata.hp_det_use_one_pin) {
		for (i = 0; i < ARRAY_SIZE(init_list_one_gpio_detect); i++)
			regmap_write(rt5659->regmap,
				init_list_one_gpio_detect[i].reg,
				init_list_one_gpio_detect[i].def);

		/*GPIO Jack Detect 1 source selection*/
		val = rt5659->pdata.gpio_jd1_sel < 0 ?
			0 : rt5659->pdata.gpio_jd1_sel;
		regmap_write(rt5659->regmap, RT5659_JD_CTRL_1,
			(val & RT5659_SEL_GPIO_JD1_MASK)
			<< RT5659_SEL_GPIO_JD1_SFT);
		val = rt5659->pdata.gpio_jd2_sel < 0 ?
			0 : rt5659->pdata.gpio_jd2_sel;
		/*GPIO Jack Detect 2 source selection*/
		regmap_write(rt5659->regmap, RT5659_JD_CTRL_2,
			(val & RT5659_SEL_GPIO_JD2_MASK)
			<< RT5659_SEL_GPIO_JD2_SFT);
	} else {
		for (i = 0; i < ARRAY_SIZE(init_list_jd3_and_gpio_detect); i++)
			regmap_write(rt5659->regmap,
				init_list_jd3_and_gpio_detect[i].reg,
				init_list_jd3_and_gpio_detect[i].def);
	}
...
struct snd_soc_dai_driver rt5659_dai[] = {
	{/* 0 */
		.name = "rt5659-aif1",
		.id = INDEX_TO_DAI_ID(RT5659_AIF1),
		.playback = {
	...
	{/* 1 */
		.name = "rt5659-aif2",
		.id = INDEX_TO_DAI_ID(RT5659_AIF2),
		.playback = {
	...
	{/* 2 */
		.name = "rt5659-aif3",
		.id = INDEX_TO_DAI_ID(RT5659_AIF3),
		.playback = {
	...
	{/* 3 */
		.name = "rt5659-aif1-ofld",
		.id = INDEX_TO_DAI_ID(RT5659_AIF1),
		.playback = {
			.stream_name = "AIF1 Playback ofld",
			.channels_min = 1,
			.channels_max = 2,
			.rates = RT5659_STEREO_RATES,
			.formats = RT5659_FORMATS,
	...
	{/* 4 */
		.name = "rt5659-aif1-fast",
		.id = INDEX_TO_DAI_ID(RT5659_AIF1),
		.playback = {
			.stream_name = "AIF1 Playback fast",
			.channels_min = 1,
			.channels_max = 2,
			.rates = RT5659_STEREO_RATES,
			.formats = RT5659_FORMATS,
	...
	{/* 5 */
		.name = "rt5659-aif1-voice",
		.id = INDEX_TO_DAI_ID(RT5659_AIF1),
		.playback = {
			.stream_name = "AIF1 Playback voice",
			.channels_min = 1,
			.channels_max = 2,
			.rates = RT5659_STEREO_RATES,
			.formats = RT5659_FORMATS,
		...
		.capture = {
			.stream_name = "AIF1 Capture voice",
			.channels_min = 1,
			.channels_max = 2,
			.rates = RT5659_STEREO_RATES,
			.formats = RT5659_FORMATS,
	...
	{/* 7 */
		.name = "rt5659-aif1-voip",
		.id = INDEX_TO_DAI_ID(RT5659_AIF1),
		.playback = {
			.stream_name = "AIF1 Playback voip",
			.channels_min = 1,
			.channels_max = 2,
			.rates = RT5659_STEREO_RATES,
			.formats = RT5659_FORMATS,
	...
	{/* 8 */
		.name = "rt5659-aif1-normal",
		.id = INDEX_TO_DAI_ID(RT5659_AIF1),
		.playback = {
			.stream_name = "AIF1 Playback normal",
			.channels_min = 1,
			.channels_max = 2,
			.rates = RT5659_STEREO_RATES,
			.formats = RT5659_FORMATS,
		...
		.capture = {
			.stream_name = "AIF1 Capture normal",
			.channels_min = 1,
			.channels_max = 2,
			.rates = RT5659_STEREO_RATES,
			.formats = RT5659_FORMATS,
...
static int rt5659_parse_dt(struct rt5659_priv *rt5659, struct device_node *np)
{
	...
	rt5659->pdata.gpio_ldo = of_get_named_gpio(np, "ldo-gpios", 0);
	rt5659->pdata.gpio_reset = of_get_named_gpio(np, "reset-gpios",
		0);

	of_property_read_u32(np, "realtek,hp-det-one-pin",
		&rt5659->pdata.hp_det_use_one_pin);

	rt5659->pdata.gpio_jd1_sel = -1;
	of_property_read_u32(np, "realtek,gpio-jd1-sel",
		&rt5659->pdata.gpio_jd1_sel);
	rt5659->pdata.gpio_jd2_sel = -1;
	of_property_read_u32(np, "realtek,gpio-jd2-sel",
		&rt5659->pdata.gpio_jd2_sel);

	rt5659->pdata.fixed_rate = -1;
	of_property_read_u32(np, "realtek,fixed-rate",
			     &rt5659->pdata.fixed_rate);
...
void rt5659_calibrate(struct rt5659_priv *rt5659)
{
	...
	regmap_write(rt5659->regmap, RT5659_SPO_AMP_GAIN, 0x1100);
	...
	/* MONO NG2 GAIN 5dB */
	regmap_write(rt5659->regmap, RT5659_MONO_GAIN, 0x0303);
...
static int setup_mclk(struct device_node *np)
{
	struct clk *mclk;
	struct clk *parent;
	struct clk *enable;

	mclk = of_clk_get_by_name(np, "mclk");
	if (IS_ERR(mclk)) {
		pr_err("%s can't get the clock dts config: mclk\n", __func__);
		return -EINVAL;
	}

	parent = of_clk_get_by_name(np, "source");
	if (IS_ERR(parent)) {
		pr_err("%s can't get the clock dts config: source\n", __func__);
		return -EINVAL;
	}

	enable = of_clk_get_by_name(np, "enable");
	if (IS_ERR(enable)) {
		pr_err("%s can't get the clock dts config: enable\n", __func__);
		return -EINVAL;
	}

	clk_set_parent(mclk, parent);
	clk_set_rate(mclk, 12288000);
...
static void rt5659_async_probe(void *data, async_cookie_t cookie)
{
	...
	struct i2c_client *i2c = rt5659->i2c;

	pr_info("%s enter\n", __func__);

	ret = setup_mclk(i2c->dev.of_node);
	if (ret) {
		pr_err("%s setup_mclk failed!\n", __func__);
		return;
	}
	msleep(300);

	rt5659->regmap = regmap_init_i2c(i2c, &rt5659_regmap);
	...
	pr_info("%s exit\n", __func__);
	if (ret)
		pr_err("%s register rt5659 codec failed!\n", __func__);
	else
		return;

error:
	regmap_exit(rt5659->regmap);
}
		    ...
		    const struct i2c_device_id *id)
{
	...
	/* Do the time-consuming work in an async way. */
	async_schedule(rt5659_async_probe, rt5659);
...
static int __init rt5659_init(void)
{
	return i2c_add_driver(&rt5659_i2c_driver);
}
fs_initcall_sync(rt5659_init);

static void __exit rt5659_exit(void)
{
	i2c_del_driver(&rt5659_i2c_driver);
}
module_exit(rt5659_exit);
